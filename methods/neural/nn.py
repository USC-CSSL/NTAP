...
def build_embedding(pretrain, train_embedding, embedding_size, vocab_size, expand_dims):
    if pretrain:
        ...
        while True:
            ...
            if done:
                ...
                if not os.path.isdir(all_params["path"]["dictionary_path"]+"/best_model"):
                    ...
                    save_path = saver.save(model.sess, all_params["path"]["dictionary_path"]+"/NTAP_model/model")
                break
...
def run_pred(model, batches, data_batches, weights, savedir, all_params):
    saver = tf.train.Saver()
    with tf.Session() as model.sess:
        if not os.path.isdir(all_params["path"]["dictionary_path"]+"/NTAP_model"):
            print("No saved model. Train a model before prediction")
        else:
            saver.restore(model.sess, all_params["path"]["dictionary_path"]+"/NTAP_model/model")
