...
import numpy as np
import operator, os, json, pickle
...
def SVM(X, Y, lookup_dict, scoring_dict, col, kfold, seed):
    ...
    choose_svm = GridSearchCV(classifier, cv=kfold, param_grid={
        'C': np.array([0.001, 0.01, 0.1, 1]),
        'gamma': np.array([0.001, 0.01, 0.1, 1])
    })
    choose_svm.fit(X, Y)
    best_model = choose_svm.best_estimator_
    scoring_dict[col][model]['params'] = choose_svm.best_params_
    for i, val in enumerate(best_model.coef_):
        print(i)
        print(type(val))
    coef_dict = {i: val for i, val in enumerate(best_model.coef_[0])}
    pickle.dump(best_model.coef_, open("coef.pkl","wb"))
    word_coefs = {lookup_dict[i]: val for i, val in coef_dict.items()}
    abs_val_coefs = {word: abs(val) for word, val in word_coefs.items()}
    top_features = sorted(abs_val_coefs.items(), key=operator.itemgetter(1), reverse=True)[:100]
    real_weights = [[word, word_coefs[word]] for word, _ in top_features]
