    ...
    def get_batches(self, corpus_ids, labels):
        ...
        for idx in range( len(corpus_ids) // self.batch_size):
            ...
            for i in range(len(text_batch)):
                while len(text_batch[i]) < max_length:
                    text_batch[i].append(padd_idx)

            yield (np.array([np.array(line) for line in text_batch]), lengths, np.array(labels_batch))
